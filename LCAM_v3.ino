...
// TODO List:
//
// [ ] WiFi: simultaneously with TFT and SD-Card
// [*] WiFi: settings from file
// [ ] WiFi: Transmitting realtime data to 
// [ ] Only connect to WiFi after command
// [ ] Capture astronomy image
//     * store columns until cancel
//     * display # of pixels captured so far
...
void uiDrawMenu() {
  ...
  tft.print("Astronomy image");
  tft.setTextColor( ILI9340_RED );
  if( menusel==4 ) tft.setTextColor( ILI9340_YELLOW );
  tft.setCursor( 50, 120 );
  tft.print("Exit menu");
}
...
void uiMenuAction( int item ) {
  switch( menusel ) {
    ...
    uiCreateConnect();
    if( !wifiInit() ) {
      tft.println(F("WiFi Network connection failed!"));
    }
    if( !wifiConnect() ) {
      tft.println(F("WiFi Service connection failed! Is App running?"));
    }
    tft.println("Press any key to continue ...");
    while( !scanKeys() )delay( 5 );
    while( scanKeys() )delay( 5 );
